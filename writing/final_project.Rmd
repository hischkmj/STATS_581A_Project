...
```{r Binary Response with Repeated Measures (Model1)}
library(geepack)
...
```{r OR Model1}
Model1_OR = format(exp(35265 * 3.21e-06), digits = 16)

```

## Count Response with Repeated Measures (Model2)
Based on Model2 (p = 0.0157), we find evidence of a difference between the odds (*KYLE CHECK*). A $35,265 increase (hiring approximately one worker) in average yearly payroll is associated with a *multiplicative* increase of 1.004 (0.4%) in predicted number of workers' compensation claim filed. 
